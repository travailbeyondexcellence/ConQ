...
func main() {
	serviceName := sharedConfig.GetEnv(sharedConfig.EnvServiceName, "instagram-connector")
	grpcPort := sharedConfig.GetEnv(sharedConfig.EnvGRPCPort, "50062")
	...
	// Register YouTube connector service here
	// pb.RegisterYouTubeConnectorServer(grpcServer, handler)
